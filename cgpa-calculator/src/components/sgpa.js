...
export default function Sgpa(props) {
  ...
  React.useEffect(() => {
    if (
      ...
      props.location.state.electiveNo === 0 ||
      props.location.state.mainNo === 0 ||
      props.location.state.labNo === 0
    ) {
  ...
  const calculateSGPA = async (e) => {
    ...
    for (
      let i = 0;
      parseInt(electiveNo) !== 1 ? i < electiveNo : i <= electiveNo;
      i++
    )
      parseInt(electiveNo) !== 1
        ? (electiveMarks +=
            Math.floor(e.target.electiveMarks[i].value / 10 + 1) * 3)
        : (electiveMarks =
            Math.floor(e.target.electiveMarks.value / 10 + 1) * 3);
    ...
    phase2No &&
      (phase2Marks = Math.floor(e.target.phase2Marks.value / 20 + 1) * 5);

    SGPA =
      (mainMarks + electiveMarks + labMarks + phase2Marks) /
      ...
      <div>
        <Container>
          ...
          >
            ...
            />
            <p style={{ textAlign: "center", color: "#1F355D", fontSize: 18 }}>
              Congrats! You've scored <span>{SGPA}</span> out of 10!
            </p>
          </Dialog>
          <Nav type="back" />
          <p style={{ textAlign: "center" }}>
            Tell us how much you scored out of a <span>100</span> here!
          </p>
